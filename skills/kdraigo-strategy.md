...
1. **Recall the SDK surface.** Call `get_sdk_docs` once at the start of a fresh task. If the user is asking about one area (clock, history fetch, order placement), pass `section` to scope it. For anything involving technical indicators, pull `section=indicators` — it lists all ~95 TA-Lib indicators and the `IndicatorManagerFor(tf).<Name>(exchange, symbol, pointType, ...)` access pattern. Don't guess indicator names or signatures; read them from there first.
